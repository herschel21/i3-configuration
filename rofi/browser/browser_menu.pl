#!/usr/bin/env perl
# Try different possible Chrome binary names
my $chrome_bin = `which google-chrome google-chrome-stable chrome 2>/dev/null | head -n1`;
chomp($chrome_bin);
die "Chrome not found. Please install Chrome browser.\n" unless $chrome_bin;
